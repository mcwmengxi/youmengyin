<template>
  <div class="main-container-tag">
    ...
    <div>
      ...
      <div :style="{ width: '100%' }" class="no-result" v-if="!selectTag">
        <svg
          role="img"
          xmlns="http://www.w3.org/2000/svg"
          xmlns:xlink="http://www.w3.org/1999/xlink"
...
// 点击指定Tag后进行选中
let selectTag: any = ref('模板')
const toggleTag = (tagTitle: string | number) => {
...
/* 跳转新的地址 */
function toNewHref(article: Record<string, any>) {
  console.log(article, window.location.origin, '==========')
  const realHref = `${window.location.origin}/youmengyin/${article.path}`
  // todo
