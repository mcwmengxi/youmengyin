...
* cz-git:  一款工程性更强，轻量级，高度自定义，标准输出格式的 commitizen 适配器。--[官方文档](https://cz-git.qbb.sh/zh/) 其它`commitizen`适配器 `cz-customizable`、`cz-conventional-changelog`
...
```cz-git``` 与 [commitlint](https://github.com/conventional-changelog/commitlint) 进行联动给予校验信息，所以可以编写于 commitlint 配置文件之中(⇒ [配置模板](https://cz-git.qbb.sh/zh/config/))。
...
export default defineConfig({
  ...
  rules: {
    'subject-case': [0], // subject大小写不做校验
    // type[scope]: [function] description msg
  ...
  prompt: {
    ...
    defaultBody: '',
    defaultIssues: '',
    defaultScope: scopeComplete, // 默认scope
    customScopesAlign: 'top',
    // customScopesAlign: !scopeComplete ? 'top' : 'bottom',
    defaultSubject: subjectComplete && `[${subjectComplete}] `, // 默认提交描述
...
commit 之后触发`pre-commit`校验提示信息, 之后`git push`推送到远程仓库
